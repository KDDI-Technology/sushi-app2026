...
const backgroundColors = ["#fcf7d1", "#ece2e1", "#e4e3cd", "#c4ddd6", "#b5f4bc"];
const avatarColors = ["#d7b89c", "#b18272", "#ec8a90", "#a1Ac88", "#99c9bd", "#50c8c6"];
...
const darken = (hex, amount) => rgb2Hex(hex2Rgb(hex).map((x) => clamp(x - amount, 0, 255)));
const hex2Rgb = (hex) =>
  hex
    ...
    .match(/.{2}/g)
    .map((x) => parseInt(x, 16));
const rgb2Hex = (rgb) => "#" + rgb.map((x) => `0${x.toString(16)}`.substr(-2)).join("");
...
const avatar = (
  ...
  {
    size = 150,
    avatarColors: avatarColors$1 = avatarColors,
    backgroundColors: backgroundColors$1 = backgroundColors,
    blackout = true,
    round = true,
  } = {},
) => {
